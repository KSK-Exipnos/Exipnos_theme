...
jQuery( document ).ready( function() {
	...
	if ( home.length === 0 ) {
		let position = window.scrollY + document.querySelector( '.entry-content' ).getBoundingClientRect().top - 400;
		console.log(position);
	}

	
	var positionY = position - 170;
		...
		if ( home.length !== 0 ) {
			...
			if ( $document.scrollTop() >= positionY ) {
				if ( jQuery( '#site-navigation .custom-logo-link' ).css( 'opacity' ) === 0 ) {
					jQuery( '#site-navigation .custom-logo-link' ).fadeTo( 200, 1,
						function() {
				...
				jQuery( '#site-navigation .custom-logo-link' ).css( 'opacity' ) === 1
			) {
				jQuery( '#site-navigation .custom-logo-link' ).fadeTo(
					200, 0,
					function() {
						jQuery( '#site-navigation .custom-logo-link' ).hide();
					}
	...
	if ( home.length !== 0 ) {
		...
		function leistungenScroll() {
			var position =
        window.scrollY +
        document.querySelector( '#leistungen' ).getBoundingClientRect().top;
			var positionY = position - 110;
		...
		jQuery( 'nav .referenzen-scroll' ).click( function( event ) {
			...
			var position =
        window.scrollY +
        document.querySelector( '#referenzen' ).getBoundingClientRect().top;
		...
		jQuery( 'nav .kontakt-scroll' ).click( function( event ) {
			...
			var position =
        window.scrollY +
        document.querySelector( '#kontakt' ).getBoundingClientRect().top;
			var positionY = position + -60;
		...
		jQuery( 'nav .anfahrt-scroll' ).click( function( event ) {
			...
			var position =
        window.scrollY +
        document.querySelector( '#anfahrt' ).getBoundingClientRect().top;
			var positionY = position - 100;
		...
		jQuery( 'nav .uns-scroll' ).click( function( event ) {
			...
			var position =
        window.scrollY +
        document.querySelector( '#uns' ).getBoundingClientRect().top;
			var positionY = position - 100;
	...
	jQuery( '.c-accordion__title' ).prepend(
		'<svg class="accordion__cirlce" x="0px" y="0px" viewBox="0 0 100 100" style="enable-background:new 0 0 100 100;" xml:space="preserve"><circle class="accordion__circle__outer" cx="50" cy="50" r="47.1"/><circle class="accordion__circle__inner" cx="50" cy="50" r="25.5"/></svg>'
	);

	// Add Dot wrapper  to accordion contents
	jQuery( '<div class="accordion-dot__wrapper"></div>' ).insertAfter(
	...
	for ( var step = 0; step < 7; step++ ) {
		...
		jQuery( '.accordion-dot__wrapper' ).append(
			'<div class="accordion-dot"></div>'
		);
	}

	//Add Accordion image Wrapper
	jQuery( '.accordion_wrapper' ).append(
		'<div class="accordion__image__wrapper"></div>'
	);
	...
	jQuery( '#leistungen__acc__wrapper .js-accordion-controller' ).click( function( index ) {
		...
		var accordion = jQuery( this ).parent(); // select parent Object
		...
		if ( accordion.hasClass( 'is-open' ) ) {
			...
			var indexOfOpen = jQuery( this ).parent().index();
	...
	function doneResizing() {
		...
		if ( WindowWidth < 820 ) {
			...
			var contentHasImg = jQuery( '.c-accordion__content' ).has(
				'.accordion__image'
			);
			...
			if ( contentHasImg.length !== 0 ) {
				...
				jQuery( '.accordion__image' ).each( function() {
					...
					var index = jQuery( '.accordion__image' ).index( this );
					// get accordion content with right index
					var appendTo = jQuery( '.c-accordion__item' )
						.eq( index )
			...
			// get descendent images of content
			var wrapperHasImg = jQuery( '.accordion__image__wrapper' ).has(
				'.accordion__image'
			);
	...
	jQuery( '.google__maps' ).append(
		'<div class="google__maps__overlay" touch="false"></div>'
	);
	...
	var $grid = jQuery( '.blocks-gallery-grid' );
	...
	( function() {
		var imgWrapper = jQuery( '.blocks-gallery-item' );
